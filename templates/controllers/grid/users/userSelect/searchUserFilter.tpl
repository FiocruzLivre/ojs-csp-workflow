...
{assign var="filterUserGroupId" value="filterUserGroupId"|concat:$filterData.gridId}
<script type="text/javascript">
	// Attach the form handler to the form.
	$('#{$formId}').pkpHandler('$.pkp.controllers.grid.users.stageParticipant.form.AddParticipantFormHandler');
	console.log('#filterUserGroupId{$filterData}')
	$('#{$filterUserGroupId}').change(function () {
		$('#{$formId}').submit();
	});
</script>
